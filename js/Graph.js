function Graph(a) {
      ...
      if (!isRising) {
        if (rise > run) {
          ...
          if(run > edgeLimit){
            ...
            else if (diff == denominator) ratio = 1;
  ...
  this.drawTwoLines = function(a){
    ...
    if (intercept < 0 && intercept > -(maxNum.update)) {
      ...
      if (slopeIsNegative) {
        ...
        startNum = intercept + maxNum.update;
  ...
  this.getAnswer = function(){
    ...
    if(numerator == 0 && denominator == 0) {
      ...
      xAnswer = _this.create('<div class="answer"><span>x</span> = '+pointContainer[0].x+'</div>');
      $("#answerHolder").append(xAnswer);
      rAnswer = _this.create('<div class="answer"><span>r</span> = 1</div>');
      ...
      yAnswer = _this.create('<div class="answer"><span>y</span> = '+this.roundTo(lr.slope,2)+'<span>x</span> + '+this.roundTo(lr.intercept,2)+'</div>');
      $("#answerHolder").append(yAnswer);
      
      if(lr.r2){
        rAnswer = _this.create('<div class="answer"><span>r</span> = '+this.roundTo(lr.r2,3)+'</div>');
        $("#answerHolder").append(rAnswer);
      }
      else {
        rAnswer = _this.create('<div class="answer"><span>r</span> = 1</div>');
  ...
  */
  ...
  this.makeEven = function(n){
    ...
    for (var i = 0; i < n; i++) {
      ...
      if (this.checkMod(updateNum)) {
        ...
        if(maxNum.update < 10) maxNum.update = 10;
        break;
      }
    }
  }
  ...
  this.checkForMaxNum = function(){
    ...
    for(var i = 0; i < pointContainer.length; i++){
      console.log(pointContainer[i].x,pointContainer[i].y);
  ...
  this.setMaxNum = function(){
    ...
    for(var i = 0; i < pointContainer.length; i++){
      if(pointContainer[i].x > pointContainer[i].y) numHolder = pointContainer[i].x;
      else numHolder = pointContainer[i].y;
      
      if (maxNum.original < numHolder) maxNum.original = numHolder;
      if( maxNum.original % 1 != 0) maxNum.original = this.roundTo(maxNum.original,0);
      /*if(maxNum.original < pointContainer[i].x){
        maxNum.original = pointContainer[i].x;
      }
      else{
        if(maxNum.original < pointContainer[i].y){
          maxNum.original = pointContainer[i].y;
          if( maxNum.original % 1 != 0) maxNum.original = this.roundTo(maxNum.original,0);
        }
      }*/
    }
    ...
    else maxNum.update = maxNum.original;
    
    console.log(maxNum.original, maxNum.update);
  }
  ...
  this.handleAddPoint = function(e){
    ...
    if(numX > maxNum.original || numY > maxNum.original){
      if(numX > numY) maxNum.original = numX;
      else maxNum.original = numY;
      
      if( maxNum.original % 1 != 0) maxNum.original = _this.roundTo(maxNum.original,0);
